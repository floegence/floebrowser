...
Use the same FloeBrowser release on both sides. Protocol version 23 distinguishes a rejected, changed pointer/wheel target from an invalid view. It also identifies directory-authorized close decisions separately from page input. It includes explicit held-input release, source-element mute, dialogs,
...
The viewer detects missing event sequences and requests a fresh snapshot. It also checkpoints after 4,000 incremental messages or 8 MiB of event text to bound rrweb replay history. A rejected action against an invalid view epoch requests one fresh snapshot without repeating input. A pointer or wheel target that moved, disappeared, was replaced or became covered is rejected before dispatch with `target_changed`; it does not invalidate the DOM stream. The viewer discards that gesture and any unsent wheel accumulation without interrupting browsing with a warning or forcing a new snapshot. The next fresh input uses the current projected target. Missing event sequences still recover through the snapshot path; input is never retargeted or replayed. Passive hover failures and timeouts, and document-bound failures or timeouts from an older view, do not produce action-failure notifications or refresh a newer view. Current-view authorization failures, unsupported operations and uncertain effect failures remain visible and are never reported as successful. A known no-effect target cancellation remains a failed acknowledgement rather than a synthetic success. Source hit testing and epoch checks still apply to every input. The `onStatus` callback reports `refreshing` while an established view synchronizes; the standalone UI keeps the current view and navigation visible, without a connection overlay. Page input resumes when the new snapshot is ready. The embedding transport owns disconnect detection.
