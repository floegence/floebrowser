...
- Canvas 2D, WebGL/WebGL2 and WebGPU graphics with source pointer, drag, wheel and keyboard input.
...
Media remains source-owned: the website obtains and decodes content at the source. Existing `srcObject` media streams are borrowed directly and their tracks are cloned; other elements use `HTMLMediaElement.captureStream()`. These individual tracks are sent to the Go `media` module's Pion collector on the same host. It binds only `127.0.0.1`, advertises only loopback candidates, uses ICE-lite, and has no STUN/TURN configuration. The optional Node adapter uses the packaged native helper; embedding Go hosts can use the collector directly. The helper uses standard input for requests, standard error for structured replies, and standard output for binary media; it has no inherited descriptor-number requirement or diagnostic output. All Pion components, including RTCP feedback interceptors, share the collector's silent logger, so closing a native transport with feedback pending cannot contaminate the control pipe with library errors. The SDK verifies the packaged native artifact checksum and negotiates matching SDK/media wire versions before accepting a collector. Neither route captures a display, tab, camera or microphone.

The collector emits bounded packets containing target, subscription, element, stream, track, timestamp, codec and keyframe identity. RTP assembly retains up to 2,048 packets so detailed keyframes fit before depacketization; its 100 ms timestamp window and 20 ms idle flush after a complete frame remain independent of that size bound. The collector maps audio and video RTP clocks onto one source timeline. Workers and playback preserve that relative timing; a late first video frame must not reset its timestamp to the first audio frame. The shared presentation clock is anchored before synchronous audio-device initialization, so device startup cannot shift later video deadlines. Device-clock sampling starts only after the worklet has consumed its first samples, avoiding a synchronous native latency query during audio startup. Presentation checks compare displayed flashes with audible pulses at the audio device clock, including initial video loss and delayed audio-device startup. The synchronized fixture in `test/fixtures/av-sync.webm` is regenerated by `node scripts/generate-sync-fixture.mjs` with ffmpeg. The remote viewer receives encoded bytes through `ProjectionConnection.subscribeMedia`, decodes video and Opus in Workers using WebCodecs, and schedules PCM through AudioWorklet. Canvas uses complete WebP element images. Identical retransmitted Canvas pictures reuse their existing decoded resource; changed pixels and source dimensions still update immediately. The client never needs source website access or a route to the collection port. Resource and replay restrictions remain unchanged.
...
Pointer-down, passive hover and ordinary click release identify the visible content node and retain source hit testing. Once a held pointer actually moves, its motion and release identify that same original node and carry normalized coordinates in its document viewport. Moving a dragged element never moves this coordinate system. The source accepts continuation only for the admitted node, current document epoch and a still-held button; every sample remains subject to host authorization, live-node resolution and containing-frame geometry/occlusion checks. Chromium owns pointer capture and native event delivery. Removed origins, replaced documents, covered frames and revoked controllers cancel the gesture. The viewer does not predict element positions or execute website drag handlers.

The viewer sends the first held move immediately and retains only the latest unsent position while it awaits acknowledgement. Pointer-up and other ordering barriers flush that final position before the next action. Failure, lost focus, navigation, snapshot replacement or disconnect discard unsent motion; old acknowledgements cannot revive it or cancel a newer gesture. Already transmitted motion is never retried. This prevents a slow source from accumulating a historical drag trail; intermediate unsent positions may be coalesced, as with browser pointer sampling. Network and source rendering latency remain visible.

Wheel input identifies the nearest scrolling container for its axes, with normalized coordinates in the frame's viewport; document scrolling identifies the document element. The viewport may remain usable even when the document element has no content box. This keeps continuous scrolling valid when preceding wheel input has already moved the content away. The source verifies that the current hit belongs to the same scroll region or an exhausted region's scroll ancestor with native chaining allowed, then maps viewport coordinates through each containing frame and checks for occlusion. Removed or replaced regions, unrelated scroll targets, stale documents and covered frames are rejected. Chromium owns native wheel handling and scroll chaining; the viewer does not scroll the website locally or retry rejected input.
...
The viewer detects missing event sequences and requests a fresh snapshot. It also checkpoints after 4,000 incremental messages or 8 MiB of event text to bound rrweb replay history. A rejected action against an invalid view epoch requests one fresh snapshot without repeating input. A pointer-down, hover or wheel target that disappeared, was replaced or became covered is rejected before dispatch with `target_changed`; it does not invalidate the DOM stream. The viewer discards that gesture and any unsent drag motion or wheel accumulation without interrupting browsing with a warning or forcing a new snapshot. The next fresh input uses the current projected target. Missing event sequences still recover through the snapshot path; input is never retargeted or replayed. Passive hover failures and timeouts, and document-bound failures or timeouts from an older view, do not produce action-failure notifications or refresh a newer view. Current-view authorization failures, unsupported operations and uncertain effect failures remain visible and are never reported as successful. A known no-effect target cancellation remains a failed acknowledgement rather than a synthetic success. Initial pointer hit testing, admitted drag-origin validation, containing-frame checks and document epochs remain enforced at the source. The `onStatus` callback reports `refreshing` while an established view synchronizes; the standalone UI keeps the current view and navigation visible, without a connection overlay. Page input resumes when the new snapshot is ready. The embedding transport owns disconnect detection.
...
The suite compares custom-property border shorthands followed by longhand overrides, such as `border: var(--line) solid; border-color: red`, against the source's computed appearance. The regression in `test/style-fidelity.e2e.ts` protects the original border when Chromium serializes pending shorthand values as empty longhands.
