...
for (const lateAttachment of [false, true])
  test(
    ...
    async (t) => {
      ...
      let service = lateAttachment
        ? undefined
        : await createProjectionServer(source, {
            authorize: () => true,
          });
      t.after(async () => {
        await service?.close();
      ...
      if (lateAttachment) {
        ...
        // A discarded pre-attachment buffer needs a native source redraw.
        await source.evaluate(() => (window as any).render([1, 0, 0, 0.5]));
      }
